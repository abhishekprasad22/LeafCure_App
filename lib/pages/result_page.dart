...
class ResultPage extends StatelessWidget {
  final dynamic image; // File, Uint8List, or String (URL)
  final Map<String, dynamic> analysisResult; // JSON data
  ...
  String _getCure(String diseaseKey) {
    switch (diseaseKey.toLowerCase()) {
      case 'brown_blight':
        return "Spray copper oxychloride (2g/liter) every 10–15 days. Improve drainage.";
      case 'gray_blight':
        return "Remove infected leaves immediately. Apply systemic fungicides like Carbendazim.";
      case 'red_rust':
        return "Apply potassium fertilizers. Spray Bordeaux mixture (1%) during dormant season.";
      case 'algal_leaf_spot':
      case 'algal_spot':
        return "Prune infected branches. Ensure proper ventilation and sunlight.";
      case 'helopeltis':
        return "Apply Thiamethoxam or Deltamethrin sprays. Monitor during early morning.";
      case 'red_spot':
        return "Apply proper fungicides and maintain soil potash levels.";
      case 'healthy':
        return "Great job! Your tea plant is healthy. Maintain good soil nutrition.";
      default:
        return "Consult a local agricultural expert for specific advice.";
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 1. Data Extraction (Safe Parsing)
    final String predictionRaw = analysisResult['final_prediction'] ?? 'Unknown';
    ...
    final double? avgTemp = usedWeather
        ? (weatherData['avg_temp_30d'] as num?)?.toDouble()
        : null;

    // 2. Image Logic Pre-calculation
    // We only convert to bytes if it's NOT a URL string
    Uint8List? localImageBytes;
    bool isNetworkImage = image is String;

    if (!isNetworkImage) {
      if (kIsWeb && image is Uint8List) {
        localImageBytes = image as Uint8List;
      } else if (image is File) {
        localImageBytes = File(image.path).readAsBytesSync();
      } else if (image is Uint8List) {
        localImageBytes = image;
      }
    }
      ...
      body: Container(
        ...
        child: SingleChildScrollView(
          ...
          child: Center(
            child: Column(
              children: [
                // 📸 Image Card (Updated for Network Support)
                Container(
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              "$voteCount out of $totalModels AI models agreed.",
                              style: TextStyle(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                ...
                                style: TextStyle(color: Colors.brown[400], fontSize: 12, fontStyle: FontStyle.italic),
                              ),
                            ],
                          ),
                        ),
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        Row(
                          children: [
                            const Icon(Icons.medical_services_outlined, color: Color(0xFF2E7D32)),
                        ...
                        Text(
                          ...
                          style: TextStyle(fontSize: isDesktop ? 16 : 14, height: 1.6, color: Colors.black87),
                        ),
                      ],
                    ),
                  ),
                ...
                ExpansionTile(
                  ...
                  children: details.map<Widget>((model) {
                    return ListTile(
                      ...
                      subtitle: Text("Transform: ${model['used_transform']} | Confidence: ${(model['confidence'] as num).toStringAsFixed(1)}%"),
                      trailing: Text(_formatName(model['prediction'] ?? ''), style: const TextStyle(fontWeight: FontWeight.bold)),
                    );
                  }).toList(),
                ),
